...
async def handle_socket(websocket, path):
    while True:
        ...
        for (i, rect) in enumerate(rects):
            ...
            print(shape[36], shape[37], shape[38],
                  shape[39], shape[40], shape[41])
            print(shape[42], shape[43], shape[44],
                  shape[45], shape[46], shape[47])

            eye1 = (mag(sub(shape[37], shape[41])) + mag(sub(shape[38],
                    shape[40]))) / (2 * mag(sub(shape[36], shape[39])))
            eye2 = (mag(sub(shape[43], shape[47])) + mag(sub(shape[44],
                    shape[46]))) / (2 * mag(sub(shape[42], shape[45])))
